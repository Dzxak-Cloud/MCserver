...
# Menanyakan domain untuk Minecraft dan FileBrowser
echo "Masukkan domain untuk Minecraft (contoh: mc.domain.com):"
read MINECRAFT_DOMAIN
...
# Membuat konfigurasi Nginx untuk Minecraft
echo "Membuat konfigurasi Nginx untuk $MINECRAFT_DOMAIN..."
cat <<EOF > /etc/nginx/sites-available/minecraft
server {
    listen 80;
    server_name $MINECRAFT_DOMAIN;
...
echo "Mengaktifkan konfigurasi Nginx untuk Minecraft dan FileBrowser..."
...
echo "Memasang SSL untuk $MINECRAFT_DOMAIN dan $FILEBROWSER_DOMAIN..."
certbot --nginx -d $MINECRAFT_DOMAIN -d www.$MINECRAFT_DOMAIN
certbot --nginx -d $FILEBROWSER_DOMAIN -d www.$FILEBROWSER_DOMAIN

# Menyelesaikan konfigurasi
echo "Konfigurasi selesai!"
echo "Minecraft dapat diakses di: https://$MINECRAFT_DOMAIN:25565"
